...
function objectAt(x, y) {
    const hit = 6;
    const rects = getComponents((c) => c instanceof Rect);
    for (const r of rects)
        if (x >= r.x - hit && x <= r.x + r.w + hit &&
            y >= r.y - hit && y <= r.y + r.h + hit)
            return r;
    const labels = getComponents((c) => c instanceof Label);
    for (const t of labels)
        if (Math.hypot(t.x - x, t.y - y) < 12)
            return t;
    return null;
}
canvas.addEventListener("pointerdown", async (ev) => {
    ...
    switch (ui.tool) {
        ...
        case "text": {
            ...
            if (value)
                addComponent(new Label(newEid(), x, y, value));
            break;
        }
        case "arrow": {
